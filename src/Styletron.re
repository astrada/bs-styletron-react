...
module Client = {
  ...
  external createInstance : (Js.nullable(array(Dom.element)), Js.nullable(Core.options)) => Core.t =
    "default";
  let make = (~serverStyles=?, ~options=?, ()) =>
    createInstance(Js.Nullable.from_opt(serverStyles), Js.Nullable.from_opt(options));
};
  ...
  let makeStyledComponent = (~rule, ~component, ~make, children) => {
    let reactClass =
      ReasonReact.wrapReasonForJs(
        ~component,
        (jsProps) => make(~className=jsProps##className, children)
      );
    let styledClass = styled(`ReactClass(reactClass), rule);
    ReasonReact.wrapJsForReason(~reactClass=styledClass, ~props=Js.Obj.empty(), children)
  };
  module Provider = {
    [@bs.module "styletron-react"] external reactClass : ReasonReact.reactClass =
      "StyletronProvider";
    let make = (~styletron=Client.make(), children) =>
      ReasonReact.wrapJsForReason(~reactClass, ~props={"styletron": styletron}, children);
  };
};
